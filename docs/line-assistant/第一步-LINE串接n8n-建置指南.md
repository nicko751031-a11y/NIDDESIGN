...
### 9.1 群組登錄機制怎麼運作（2026-08-26 起改為自動核准）

新群組**自動核准、立即開始記錄**，不需要人工核准：

1. 把「川果設計 NID DESIGN LAB」(`@549mpmon`) 邀請進群組
2. 群組裡發一則訊息
3. n8n 發現這個群組沒登錄 → 自動在「02｜群組登錄」開一列，狀態直接設為 **「已登錄」**
4. **同一則訊息**就會繼續往下走完整流程（去重 → AI 分類 → 寫入 Notion／Dropbox），不會漏掉第一則

之後可再到 Notion 補上這些欄位（選填，補了會更好用）：

- 群組類型（業主群／工程群／設計群…）
- 所屬專案（關聯到 01｜專案）
- 排序權重（給第二步的週報用）
- 摘要重點指示（會餵給 AI，例如「此群重點在進場排程與缺失改善」）

> **不想記錄的群組**：把該列的「註冊狀態」改成 **「停用」**（任何非「已登錄」的值都會擋掉），
> 之後該群組的訊息與檔案都不會再被記錄。
>
> 早期版本預設狀態是「待確認」、需人工核准；若要改回嚴格模式，把
> 「組出待確認群組」節點裡的 `'已登錄'` 改回 `'待確認'`，並移除
> 「Notion｜登錄待確認群組 → Notion｜檢查重複訊息」這條連線即可。
...
訊息經 AI 分類時若推得「到期時間」（例如「明天下午三點丈量」「9/4 前出圖」），
會自動在 Google「川果設計」日曆建立事件：

- **標題**＝AI 摘要；**說明**＝來源群組／發送者／原始訊息；**時長**＝1 小時
- 節點：「整理分類結果」→「IF｜需建立行事曆」→「Google Calendar｜建立事件」，
  與 Notion 寫入平行執行，建立失敗不影響訊息紀錄
- 憑證：n8n 的「Google Calendar」（Google Calendar OAuth2 API）。
  Google Cloud 端的 OAuth 用戶端建立在專案 GWS-CLI，
  重新導向 URI 為 `https://<你的網域>/rest/oauth2-credential/callback`
- ⚠️ 若 OAuth 同意畫面的發布狀態是「測試中」，授權每 7 天過期，
  需重按一次 Sign in；發布成「實際運作中」即永久有效
- 只想換目標日曆：改「Google Calendar｜建立事件」節點的 Calendar 下拉選單即可
