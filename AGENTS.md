...
**線上現況（2026-08-27）**：工作流 01 共 40 節點，已發布。
repo 的 `n8n/workflows/01-line-ingest.json` 與線上一致。
已上線的功能：LINE 群組與私訊歸檔、AI 文字分類、照片視覺分析、
Dropbox 備份與分享連結、Google 行事曆事件、新群組自動核准。

短期待辦：
- 清理臨時工作流 `ZZ5`（`ky3cDvih19h2GWWE`）；`ZZ6` 已封存
- 清理更早的 `ZZ2`／`ZZ3`（內含舊 n8n API key 明文）
- 輪換 n8n API key（曾在對話中明文出現過）
- PR #2 尚未合併；合併後 `AGENTS.md` 才會出現在預設分支
